...
public class InfoPageButton extends Widget implements IHoverTextWidget {
    ...
    protected ITextComponent getNarrationMessage(){
        return new TranslationTextComponent("gui.tesseract.info.narrate_page", this.page.get().getTitle());
    }

    @Override
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks){
        ScreenUtils.bindTexture(BUTTONS);
        ScreenUtils.drawTexture(matrixStack, this.x, this.y, this.width, this.height, 7 / 18f, (this.active ? this.hovered ? 1 : 0 : 2) / 3f, 4 / 18f, 1 / 3f);
    }
}
