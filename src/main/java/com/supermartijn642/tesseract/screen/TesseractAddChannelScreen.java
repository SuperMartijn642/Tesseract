...
public class TesseractAddChannelScreen extends TileEntityBaseScreen<TesseractTile> {
    ...
    protected TesseractAddChannelScreen(BlockPos pos, EnumChannelType type){
        super(new TextComponentTranslation("gui.tesseract.add.title." + type.name().toLowerCase(Locale.ROOT)), pos);
        this.type = type;
    }
    ...
    protected void addWidgets(TesseractTile tile){
        ...
        this.addButton = this.addWidget(new TesseractButton(8, 43, 61, 14, new TextComponentTranslation("gui.tesseract.add.add"), this::addChannel));
        this.addButton.active = false;
        TesseractButton cancelButton = this.addWidget(new TesseractButton(75, 43, 61, 14, new TextComponentTranslation("gui.tesseract.add.cancel"),
    ...
    @Override
    protected void render(int mouseX, int mouseY, TesseractTile tile){
        ScreenUtils.bindTexture(BACKGROUND);
        ScreenUtils.drawTexture(0, 0, this.sizeX(), this.sizeY());

        ScreenUtils.drawCenteredString(this.title, 72, 6, 0xffffffff);
    }
