...
public class TesseractRemoveChannelScreen extends BlockEntityBaseWidget<TesseractBlockEntity> {
    ...
    protected void render(int mouseX, int mouseY, TesseractBlockEntity entity){
        ...
        ScreenUtils.drawCenteredString(TextComponents.translation("gui.tesseract.remove.title." + this.type.name().toLowerCase(Locale.ROOT)).get(), 72, 6, 0xffffffff);
        // Render player head and channel name
        int nameWidth = ClientUtils.getFontRenderer().getStringWidth(channel.name);
        int x = 72 - (9 + 3 + nameWidth + 3 + 9) / 2;
        PlayerRenderer.renderPlayerHead(channel.creator, x, 24, 9, 9);
        ScreenUtils.drawString(channel.name, x + 12, 25, 0xffffffff);
        if(channel.creator.equals(ClientUtils.getPlayer().getUniqueID())){
            GlStateManager.enableAlpha();
            ScreenUtils.bindTexture(channel.isPrivate ? TesseractScreen.LOCK_ON : TesseractScreen.LOCK_OFF);
            ScreenUtils.drawTexture(x + 12 + nameWidth + 3, 24, 9, 9);
        }
